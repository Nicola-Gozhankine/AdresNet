...
void compute_metrics(const uint8_t* buffer, int num_bytes) {
    int total_bits = num_bytes * 8;
    int ones = 0;
    ...
    for (int i = 0; i < total_bits-1; ++i) {
        ...
        int byte2 = (i+1)>>3, bit2 = 7-((i+1)&7);
        if (((buffer[byte1]>>bit1)&1) == ((buffer[byte2]>>bit2)&1)) same1++;
    }
    double corr1 = std::abs(same1 - (total_bits-1)/2) / (double)(total_bits-1);

    int same8 = 0;
    for (int i = 0; i < total_bits-8; ++i) {
        int byte1 = i>>3, bit1 = 7-(i&7);
        int byte2 = (i+8)>>3, bit2 = 7-((i+8)&7);
        if (((buffer[byte1]>>bit1)&1) == ((buffer[byte2]>>bit2)&1)) same8++;
    }
    double corr8 = std::abs(same8 - (total_bits-8)/2) / (double)(total_bits-8);
    ...
    std::cout << "Frequency deviation: " << freq_dev*100 << "%" << std::endl;
    std::cout << "Max run length: " << max_run << std::endl;
    std::cout << "Autocorr lag1 deviation: " << corr1*100 << "%" << std::endl;
    std::cout << "Autocorr lag8 deviation: " << corr8*100 << "%" << std::endl;
    std::cout << "Poker chi^2: " << chi2 << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        compute_metrics(buffer, TEST_BYTES);
